...
export class DriversController {
    ...
    async create(@Body() dto: CreateDriverDto, @Res() res: Response) {
        const { status, ...responseData } = await this.driversService.create(dto);
    ...
    async remove(@Param('id') id: string, @Res() res: Response) {
        const { status, ...responseData } = await this.driversService.remove(id);
        return res.status(status).json(responseData);
    }
}

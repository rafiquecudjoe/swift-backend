...
export class VehiclesController {
    ...
    async create(@Body() dto: CreateVehicleDto, @Res() res: Response) {
        const { status, ...responseData } = await this.vehiclesService.create(dto);
    ...
    async remove(@Param('id') id: string, @Res() res: Response) {
        const { status, ...responseData } = await this.vehiclesService.remove(id);
        return res.status(status).json(responseData);
    }
}
